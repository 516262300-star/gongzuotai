...
- 左侧导航：状态总览、Agent 历史、任务执行。
- 状态总览：读取四个现有项目的最新日志或保存历史，顶部显示成功、失败、警告、未运行数量。
- Agent 历史：按 agent 下拉选择，显示最近 20 条运行、周报、同步、草稿或上架日志记录。
- 任务执行：提供预览和执行按钮，除 `status` 外真实执行都需要输入 `EXECUTE`；运行中会锁定按钮、显示已运行秒数，并实时滚动显示脚本 stdout/stderr 输出，避免用户误以为卡住后重复点击。
